...
fn test_initialize() {
    ...
    let counter = Pubkey::find_program_address(
        &[q3_26_vault::constants::COUNTER_SEED],
        &program_id,
    )
    .0;
    ...
    let counter_state = q3_26_vault::state::Counter::try_deserialize(&mut data).unwrap();
    assert_eq!(counter_state.count, 0);
    assert_eq!(counter_state.authority, payer.pubkey());

    let instruction = Instruction::new_with_bytes(
        program_id,
        &q3_26_vault::instruction::Increment {}.data(),
        q3_26_vault::accounts::Increment {
            counter,
